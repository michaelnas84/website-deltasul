...
function excluir(reg) {
  $.ajax({
      data: { registro: reg, acao: 'blog_excluir' },
      type: "POST",
      ...
      success: function (retorno) {
        if (retorno == 'error_user') {
            ...
            exibe_notificacao('red', 'Post excluído com sucesso!', 'A página será recarregada')
            setTimeout(() => window.location.reload(), 2000)
        }
      }
  })
...
function confirmar(reg) {
  $.ajax({
      data: { registro: reg, acao: 'blog_confirmar' },
      type: "POST",
      url: 'includes/central_controller.php',
      method: 'POST',
      success: function (retorno) {
        if (retorno == 'error_user') {
            ...
            exibe_notificacao('green', 'Post confirmado com sucesso!', 'A página será recarregada')
            setTimeout(() => window.location.reload(), 2000)
        }
      }
  })
...
function exibe_modal(a) {
  $('#title-modal-exibe-imagem').text(a.id)
  $('#img-modal').attr("src", a.firstElementChild.src)
}
