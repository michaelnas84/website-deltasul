<?php
    ...
    if($_SESSION['nome_usuario'] == null || $_SESSION['permissoes'] == null || (!in_array("7", $_SESSION['permissoes']))){
      ...
      SELECT 
         A.referencia               AS REF,
         B.descricao				        AS DESCR,
         COUNT(A.referencia)        AS REF_QTD
      FROM
         web_buscaproduto A,
         web_produto B
      WHERE 
      ...
      GROUP BY 
         A.referencia
      ORDER BY 
         COUNT(A.referencia) DESC
      ";
      // echo '<pre>' . $sql . '</pre>'; exit;
      $stmt = $pdo->query($sql);
      $tot_itens=0;
      while($row_produtos_interessantes = $stmt->fetch()){
        $prod_mais_interessante[]                    = $row_produtos_interessantes['REF'] . ' - ' . $row_produtos_interessantes['DESCR'];
...
<head>
  ...
  <title>Clicks de Interesse | Lojas Deltasul</title>
...
<?php $header_page_1 = 'Clicks de Interesse';
      $header_page_1_url = 'dashboard_clicks_de_interesse.php';
      $header_page_name = 'Dashboard Clicks de Interesse';
...
<div class="w-full overflow-hidden bg-white shadow sm:rounded-md">
  ...
  <?php for($xy=0; $xy<count($prod_mais_interessante); $xy++){ ?>
    <li>
      <a class="block hover:bg-gray-50">
        <div class="px-4 py-4 sm:px-6">
          <div class="mt-2 sm:flex sm:justify-between">
            <p class="truncate text-sm font-medium text-blue-600 hover:text-blue-800 transition-all"><?= $prod_mais_interessante[$xy]; ?></p>
            ...
            <svg class="mr-1.5 h-5 w-5 flex-shrink-0 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
              <p>
                <time><?= $qtd_prod_mais_interessante[$xy]; ?></time>
              </p>
            </div>
          </div>
        </div>
...
<script src="js/dashboard_clicks_de_interesse.js"></script>